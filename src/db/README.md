...
`db:seed` で50店舗と新仕様の口コミ214件を用意します。各店舗には3〜5件を割り当て、主要24店舗は詳細な投稿者属性を表示できる5件以上、それ以外は匿名性を優先した粗い属性表示を確認できる5件未満にします。各口コミには属性、雰囲気・スタッフ層のタグ、内容と対応する4項目評価、生の声を設定します。個別の参考画像は追加しません。旧ダミー口コミからの移行は [開発手順](../../doc/review-flow-development.md) に従ってください。

追加店舗はカフェ・コンビニ・教育の3業種、東京・神奈川・埼玉・千葉の12エリアに分散しています。店舗名と本文にデモ・架空の表示を付け、実在店舗への口コミと誤認しないようにしています。住所も架空の表記に固定しています。

ダミー投稿者5名は固定テストユーザーと分けています。seedは既存口コミを削除・上書きせず、固定IDの不足分だけを追加します。既存口コミに紐づくEmbeddingも保持し、seed自身は新しいEmbeddingを生成しません。再実行しても件数は重複しません。対象は確認済みの開発用DBだけにしてください。
